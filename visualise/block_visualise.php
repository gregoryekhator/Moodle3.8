...
require_once($CFG->dirroot.'/blocks/visualise/lib.php');

class block_visualise extends block_base {

    public function init() {
        $this->title = get_string('pluginname', 'block_visualise');
    }
    ...
    public function get_content() {
        ...
        $PAGE->requires->css('/blocks/visualise/scripts/style/jsmind.css',true);
        $PAGE->requires->jquery();
        $PAGE->requires->js('/blocks/visualise/scripts/js/drag-on.js',true);
        $PAGE->requires->js('/blocks/visualise/scripts/js/jsmind.js',true);
        $PAGE->requires->js('/blocks/visualise/scripts/js/jsmind.launcher.js',true);
        ...
        $content = visualise_content_base();
        ...
        if ($nbr_courses > 0)
        {
          $opened_course = visualise_last_course();
          foreach ($courses as $course)
          {
              //this is mandatory in order to make a comparison at the end of the cycle
             $nbcmod = visualise_coursemod_query($course->id);
          ...
          foreach ($courses as $course)
          {
            ...
            if ($nbcmod == 0 && $course->format != 'social')
                continue;
            $rolecourse = visualise_my_role_course($course);
            $numcourse++;
            if ($course->visible == 0)
            {
               $courseskip++;
            ...
            if ($nbcmod > 0 || $course->format == 'social')
            {
              ...
              {
                    $section_name = ($section->name === NULL || trim(strip_tags($section->name)) == '') ? get_string('mymindmap_withoutname','block_visualise') : addslashes($section->name);
                   ...
                   $modseq = array_values(visualise_mod_sections($nb_modtot,$course,$modseq));
                   $nb_mod = count($modseq);
                   $newidseq = 1;
                   $counter = 0;
                   if ($nb_mod > 0)
                   {
                      for ($i=0;$i < $nb_mod; $i++)
                      {
                         ...
                         foreach ($modules as $record)
                         {
                             ...
                             $is_member = visualise_ismember($rolecourse,$record);
                             $counter++;
                             ...
                             if (file_exists($CFG->dirroot.'/mod/'.$module.'/pix/icon.gif'))
                              ...
                              if ($is_member == 0)
                                 ...
                                 preg_replace("#\n|\t|\r#",'',$mod_name).
                                 get_string('mymindmap_restriction','block_visualise').'"}'.$virgule;
                             else
                                 $content1 .= '
            ...
            if ($is_passed == 1 && $nbpassed == 1 && $totactual > 0)
                  ...
                  $totpassed.') '.get_string('mymindmap_past','block_visualise').'</span>'.
         ...
         $this->content->text .= html_writer::div(visualise_screenview ($content,$hight));
       }
       else
       {
         $this->content->text = html_writer::div(visualise_no_courses);
       }
       $this->content->footer = '';
       return $this->content;
    }
